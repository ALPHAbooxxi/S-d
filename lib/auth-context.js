...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // When the PWA returns from the app switcher / background,
    // re-validate and refresh the session so the user stays logged in.
  ...
  const register = useCallback(async ({ username, displayName, email, password }) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        emailRedirectTo: getAuthRedirectPath('/auth/confirm?next=/dashboard'),
        data: {
