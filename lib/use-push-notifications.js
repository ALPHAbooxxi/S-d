...
export function usePushNotifications() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      void ensureSubscription().catch((err) => {
  ...
  }, [busy, ensureSubscription, permission, serverSubscriptionCount, supported])
  ...
  }, [ensureSubscription, permission, supported])
  ...
  const reconnectPush = useCallback(async () => {
    try {
      ...
      setError('')
      setMessage('')
      return await ensureSubscription({ forceResubscribe: true })
    } catch (err) {
