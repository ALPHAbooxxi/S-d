...
export function TradesProvider({ children }) {
  ...
  const createCounterOffer = useCallback(async (originalTradeId, receiverId, offeredStickers = [], wantedStickers = [], message = '') => {
    if (!user) return null

    // 1. Create the new trade request (Counter Offer)
    const { data: newTradeData, error: newTradeError } = await supabase
      .from('trade_requests')
      .insert({
        sender_id: user.id,
        ...
        message: message?.trim() || 'Gegenangebot',
        status: 'offen',
      })
      .select('id, sender_id, receiver_id, offered_stickers, requested_stickers, message, status, created_at, updated_at')
      .maybeSingle()
    ...
    if (newTradeError) {
      throw new Error(newTradeError.message)
    }

    if (newTradeData) {
      const nextTrade = mapTrade(newTradeData)
      syncTrades((current) => upsertById(current, nextTrade))

      // 2. Automatically decline the old trade
      const { data: oldTradeData, error: oldTradeError } = await supabase
        .from('trade_requests')
      ...
      if (!oldTradeError && oldTradeData) {
        syncTrades((current) => upsertById(current, mapTrade(oldTradeData)))
      }

      await notifyPush({
        receiverId,
        title: `${user.displayName || user.username} hat ein Gegenangebot gemacht`,
        body: message?.trim() || 'Oeffne den Chat, um das Gegenangebot anzusehen.',
